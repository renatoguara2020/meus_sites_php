...
<table class="table mt-3">
	...
	if($txtbuscar == ''){
		$res = $pdo->query("SELECT * from cargos order by id desc LIMIT $limite, $itens_por_pagina");
	}else{
		$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
		$res = $pdo->query("SELECT * from cargos where nome LIKE '$txtbuscar' order by id desc");
